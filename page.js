    ...
    <main style={{ background: "#f6f7fb", minHeight: "100vh" }}>
      ...
      <Section id="reis" title="🚗 Reisinfo">
        <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 1.6 }}>
          <li>
            <strong>Verzamelpunt:</strong> {data.travel.meetingPoint}
          </li>
          <li>
            <strong>Vertrek:</strong> {data.travel.departTime} &nbsp;|&nbsp; <strong>Aankomst:</strong> {data.travel.arrivalTime}
          </li>
        </ul>
        <ul style={{ marginTop: 10, paddingLeft: 18, lineHeight: 1.6 }}>
          {data.travel.notes.map((n) => (
            <li key={n}>{n}</li>
          ))}
        </ul>
      </Section>
      ...
      <Section id="verblijf" title="🏠 Verblijf">
        <p style={{ marginTop: 0 }}>
          <strong>{data.stay.name}</strong>
          <br />
          {data.stay.address}
        </p>
        <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 1.6 }}>
          <li>
            <strong>Incheck:</strong> {data.stay.checkin}
          </li>
          <li>
            <strong>Uitcheck:</strong> {data.stay.checkout}
          </li>
          <li>
            <strong>Contact:</strong> {data.stay.contact}
          </li>
        </ul>
        <ul style={{ marginTop: 10, paddingLeft: 18, lineHeight: 1.6 }}>
          {data.stay.notes.map((n) => (
            <li key={n}>{n}</li>
          ))}
        </ul>
      </Section>
          ...
          {packingItems.map((item) => (
            <label
              ...
              style={{
                ...
                borderRadius: 12,
                background: checked[item] ? "#e6fffa" : "white"
              }}
            >
      ...
      <footer style={{ padding: "22px 18px 40px", maxWidth: 980, margin: "0 auto", color: "#666" }}>
        <small>
          Tip: bewerk de teksten in <code>app/page.js</code> bovenin bij <code>data</code> en deploy opnieuw via GitHub → Vercel.
        </small>
      </footer>
    </main>
  );
